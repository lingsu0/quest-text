import type { Question, ExamMode, ExamResult, QuestionFilter, QuestionType, Difficulty } from './types.js';
import { 
...
let currentMode: ExamMode = 'practice';
...
function renderQuestionList(): void {
  ...
  const filter: QuestionFilter = {
    type: (document.getElementById('filter-type') as HTMLSelectElement)?.value as QuestionType || undefined,
    difficulty: (document.getElementById('filter-difficulty') as HTMLSelectElement)?.value as Difficulty || undefined,
    keyword: (document.getElementById('search-keyword') as HTMLInputElement)?.value || undefined
  };
 ...
 * 开始考试
 */
function startExam(mode: ExamMode): void {
  currentMode = mode;
  clearExamState();
 ...
 * 绑定选项事件
 */
function bindOptionEvents(question: Question): void {
  const container = document.getElementById('question-container');
  if (!container) return;
...
function updateProgress(): void {
  ...
  if (statCorrect) statCorrect.textContent = String(correctCount);
  if (statWrong) statWrong.textContent = String(wrongCount);
}
...
function updateNavigation(): void {
  ...
  for (let i = 0; i < total; i++) {
    ...
    else if (status === 'correct') className += ' correct';
